...
app.get('/cdn/*', function(req,res){
	var f=path.join(__dirname, 'cdn', req.params[0])//alertify.js')
	res.sendfile(f)
})
...
app.get('/delete/*', get_parent_path, function(req,res){
	fs.unlink(req.target_path, function(err){
		if (err) res.send(err)
			...
			debug('go path', req.parent_path)
			res.redirect(req.parent_path)
		}
	})
})

app.post('/delete/*', get_parent_path, function(req,res){
	fs.unlink(req.target_path, function(err){
		if (err) res.send(err)
		else res.send('done')
	})
...
app.get('/file_image/:file_name', function(req,res){
	...
	debug('ext',req.params.file_name+'===>'+ext)		
...
function render_items(req,res){
	...
	if (req.query.debug)
		...
		res.render('fexp.jade', {
			...
			parent_path:parent(req.base_dir)
		})//send(req.items)
}
...
app.get('/', function(req,res){
	get_drive(function(disks){
		var disks2=_.sortBy(disks, ['mountpoint'])

		res.render('disk.jade',{
			tables:[{
				title:'disk list', 
				data:disks2
			}]
...
get_drive(function(c){console.log(c)})